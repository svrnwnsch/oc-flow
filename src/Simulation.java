...
public class Simulation extends SimState {
	...
	// Durch die Methode start() wird das Feld mit den Ameisen und Gegenst�nden initialisiert
	public void start() {
		super.start();
		// Das Feld, auf dem die Agenten initialisiert werden, wird geleert.
		area.clear();

		// Eine Instanz der Entropy-Klasse wird erzeugt und dem Schedule hinzugef�gt.
		// Der Schedule sorgt daf�r, dass die step()-Methode des Entropy-Objekts jede 1.0te Zeiteinheit auf-
		...
		// Es werden neue Ameisen erzeugt, denen eine Position �bergeben wird. Diese Ameisen werden 
		...
		for (int i = 0; i < Config.numAnts; i++) {
			Human ant = new Human(position);
			area.setObjectLocation(ant, position);
			// scheduleRepeating(ant) sorgt daf�r, dass jeder Agent jede 1.0te Zeiteinheit (1.0, 2.0, 3.0 ...)
			// einen Schritt macht.
			// Alle Agenten machen dies gleichzeitig und in zuf�lliger Reihenfolge.
		...
		for (int i = 0; i < Config.numObstacles; i++) {
			position = null;
			do {
				// Zuf�llig Position wird generiert.
				position = new Int2D(
	...
	public static void main(String[] args) {
		 ...
		 * Simulationsschritte:
		 * 1. Erzeugen einer Instanz der SimState Klasse, der eine Zufallszahl �bergeben wird.
		 * 2. Die start Methode wird aufgerufen, in der die Simulation initialisiert wird.
		 * 3. Die step Methode wird immer wieder aufgerufen was daf�r sorgt, dass die Agenten (wie im Schedule/Ablaufplan der Simulation vorgegeben) 
		 * 		einen Schritt/Step t�tigen.
		 * 4. Wenn der Schedule keine Agenten mehr enth�lt / oder nach N Schritten wird die Simulation beendet.
		 */
